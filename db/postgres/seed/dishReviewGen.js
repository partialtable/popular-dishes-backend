...
const dataGen = () => {
  writer.pipe(fs.createWriteStream('./db/postgres/seed/data/reviews8.csv'));

  for (let i = counter; i < 20000001; i++) {
  // for (let i = 0; i < 10; i++) {
    ...
    writer.write({
      ...
      review: faker.lorem.paragraph(Math.floor(Math.random() * (6 - 4) + 4)),
      dined_on: fakeDate,
